...
def chunk(method="semantic-split"):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        try:
            ...
            print(
                f"Warning: Unable to parse tags for "
                f"{row['title']}. Using empty string."
            )
            tags = ""
...
def embed(method="semantic-split"):
    ...
    data_df = data_df[
        data_df["chunk"].notna() & (data_df["chunk"].str.strip() != "")
    ]
    ...
    batch_size = 15 if method == "semantic-split" else 50
    embeddings = generate_text_embeddings(
        chunks, EMBEDDING_DIMENSION, batch_size=batch_size
    )
...
def load(method="semantic-split"):
    ...
    jsonl_file = os.path.join(
        OUTPUT_FOLDER, f"embeddings-{method}-songs.jsonl"
    )
    print("Processing file:", jsonl_file)
...
def query(method="semantic-split"):
    ...
    query = (
        "I need a chill playlist for studying with "
        "ambient and lo-fi beats to help me focus."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--embed", action="store_true", help="Generate embeddings"
    )
    parser.add_argument(
