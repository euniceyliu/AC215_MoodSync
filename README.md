...
For this milestone, we created the slides and presented a midterm presentation in business pitch style to the class (it is located in the reports folder). In addition, we enhanced our RAG infrastructure by integrating our finetuned model and incorporating an agent component into the llm_rag system. This update leverages the improved performance of our specialized model while adding agent capabilities to make the system more dynamic and interactive. These enhancements allow for more sophisticated query handling and contextual understanding, ultimately providing more accurate and relevant responses by combining the strengths of our finetuned model with the agent's ability to orchestrate complex information retrieval and generation tasks. During this phase, we extensively experimented with various text chunking strategies, including semantic chunking based on content meaning, recursive chunking for nested document structures, and simple character-based splitting. Our experimentation particularly focused on different content combinations for each chunk - comparing the effectiveness of chunks containing only song lyrics versus chunks that combined lyrics with their corresponding annotations and song metadata (such as artist, title, and release date).
...
## LLM Experiments
**LLM RAG Experiments**
1. Within the datapipeline folder, the containers built from running docker-shell.sh performes chunking, embedding, loading, query for the dataset. Specific instructions can be found within the folder.
2. Within the llm-rag folder, the containers built from running docker-shell.sh performs loading and chatting with the LLM+RAG model. Specific instructions can be found within the folder.
3. Attempts at preliminary RAG experimentation with semantic chunking, different temperatures, different prompts, and different chatting content retrieved from RAG search can be found at [this google sheets](https://docs.google.com/spreadsheets/d/1y8O647Cm27uGKXFjlYm7Tbsdz7yxjr2rYflDZmshVo4/edit?gid=1856295283#gid=1856295283). For a summary of the experiments, please see [experiment documentation](results/experiments).
